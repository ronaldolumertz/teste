...
export default function DefaultEntryModal({ company, currentColumnId, warningMsg, onClose, onSaved }) {
  ...
  const save = async () => {
    setSaving(true)
    localStorage.setItem(`default_column_${company.id}`, selected || '')
    await supabase.from('companies').update({ default_column_id: selected }).eq('id', company.id)
    setSaving(false)
    onSaved(selected)
    onClose()
  }
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal confirm-modal" role="dialog">
        <div className="modal-header">
          ...
          <span className="modal-title">Entrada padrão</span>
        ...
        <div className="modal-body">
          {warningMsg && (
            <div className="default-entry-warning">
          ...
          {loading ? (
            ...
            <div className="default-entry-empty">
              Cadastre uma etapa primeiro para definir a entrada padrão.
            </div>
          ) : (
              ...
              {columns.map(col => {
                ...
                return (
                  <label key={col.id} className={`default-entry-item${selected === col.id ? ' selected' : ''}`}>
                    ...
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ opacity: .35, flexShrink: 0 }}>
